...
while norm(dk) > epsilon
    ...
    dk = pinv([hessian_p2(x) A';A 0])*[gradient_p2(x);0];
    dk(4,:) = [];
    t = 1;
    xtemp = x - t * dk;
...
f_p2 = @(x) exp(x(1)+x(2)+x(3)) + x(1)^2 + 2*x(2)^2 + 3*x(3)^2 - 2*x(1) - 7*x(2) - 5*x(3);

gradient_p2 = @(x) [exp(x(1)+x(2)+x(3))+2*x(1)-2;
                    exp(x(1)+x(2)+x(3))+4*x(2)-7;
                    exp(x(1)+x(2)+x(3))+6*x(3)-5];
hessian_p2 = @(x) [exp(x(1)+x(2)+x(3))+2 exp(x(1)+x(2)+x(3))   exp(x(1)+x(2)+x(3));
